				...
				<el-table-column label="操作" align="center" width="200">
					<template slot-scope="scope">
						...
						<el-button type="danger" @click="del(scope.row.id)" style="margin-left: 10px;" size="mini"
							icon="el-icon-delete"
							v-if="btnAuthen.permsVerifAuthention(':netWorkDot:charge:timeCharge:delete')">
							删除
						</el-button>
					</template>
				</el-table-column>
...
<script>
	...
	import addPage from './components/timeCharge/addPage.vue'
	import editPage from './components/timeCharge/editPage.vue'
	export default {
		components: {
			addPage,
			editPage
							...
							list.forEach((item, index) => {
								if (item.priceContents.length != 0) {
									...
									item.priceContents.forEach((item1, index1) => {
										let detail = item1.duration + '小时' + "/" + item1.money + "元"
										arr[index1] = detail
