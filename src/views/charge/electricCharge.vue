				...
				<el-table-column label="操作" align="center" width="200">
					<template slot-scope="scope">
						...
						<el-button type="danger" @click="del(scope.row.id)" style="margin-left: 10px;"
							icon="el-icon-delete"
							v-if="btnAuthen.permsVerifAuthention(':netWorkDot:charge:electricCharge:delete')"
							size="mini">删除
						</el-button>
					</template>
				</el-table-column>
...
<script>
	...
	import addPage from './components/electricCharge/addPage.vue'
	import editPage from './components/electricCharge/editPage.vue'
	export default {
		components: {
			addPage,
			editPage
							...
							list.forEach((item, index) => {
								if (item.priceContents.length != 0) {
									item.priceContents.forEach((item1, index1) => {
										let detail = item1.duration + '度电' + "," + item1.money + "元"
										console.log(detail)
										arr[index1] = detail
									})
