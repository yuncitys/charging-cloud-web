<template>
	<div style="display: inline-block;">
		...
		<el-drawer
			...
			<el-form ref="formData" :model="formData" label-position="left" label-width="80px" class="charge-scheme-form">
					...
					<div style="margin: 30px 0;margin-left: 20px;">
						<div v-for="(item, index) in formData.duration" :key="index" class="formItem">
							<div style="display: flex;align-items: center;">
								<div class="power-charge-form__group">
					...
					<div>
						...
						<div class="charge-scheme-settings__content">
							...
							<div v-if="isAutostop">
								...
								<div class="charge-scheme-settings__row" v-if="pageType===2">
								...
								<div class="charge-scheme-settings__row">
									...
									<div class="charge-scheme-settings__control">
										<el-input class="charge-scheme-settings__input" placeholder="请输入时间" v-model="maxChargeTime" type="number">
											<template slot="append">小时</template>
										</el-input>
		...
		methods: {
			showDidlaoformData() {
				...
				if (item.priceContents != 0) {
					item.priceContents.forEach((val, index) => {
						ids.push(val.id)
						let obj = {
							left: val.powerSectionBefore,
							right: val.powerSectionAfter
						}
						let obj1 = {
							hour: val.duration,
			...
			onformData(formName) {
				...
				this.$refs[formName].validate(valid => {
					...
					if (valid) {
						console.log("通过")
						updateDevicePrice(resultData).then(res => {
							if (res.code == 200) {
