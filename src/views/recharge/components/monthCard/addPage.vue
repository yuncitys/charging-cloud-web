		...
		<el-dialog :visible.sync="showAdd" title="新增月卡套餐" @close="showAdd = false" :append-to-body="true">
			<el-form ref="addData" :model="addData" label-position="left" label-width="100px" :rules="rules">
				...
				<el-form-item :label="'运营商户'" prop="tenantId">
					<el-select style="width: 100%;" class="filter-item" v-model="addData.tenantId" filterable clearable placeholder="请选择运营商" @change="handleChange">
						<el-option
				...
				<el-form-item :label="'运营站点'" prop="chargingStationId">
					...
					filterable clearable :disabled="isDisabled" placeholder="请选择充电站">
						<el-option v-for="item in chargingStationList"
							:key="item.id"
						...
						<div style="display: flex;align-items: center;justify-content: space-between">
							<div style="display: flex;">
								...
								<div style="margin-left: 10px">
									<el-input placeholder="请输入次数" v-model="addData.monthTotal" type="number">
							...
							<div style="display: flex;" >
								<span>单日总次数</span>
								<div style="margin-left: 10px">
									<el-input placeholder="请输入次数" v-model="addData.dayTotal" type="number">
										<template slot="append">次</template>
									</el-input>
								</div>
							</div>
					...
					<el-form-item v-if="addData.chargingMonthType === 2">
						<div style="display: flex;align-items: center;justify-content: space-between">
							<div style="display: flex;">
								<span>单月总时长</span>
								<div style="margin-left: 10px">
									<el-input placeholder="请输入时长" v-model="addData.monthTotal" type="number">
							...
							<div style="display: flex;" >
								<span>单日总时长</span>
								<div style="margin-left: 10px">
									<el-input placeholder="请输入时长" v-model="addData.dayTotal" type="number">
										<template slot="append">分钟</template>
									</el-input>
								</div>
							</div>
				...
				<el-form-item :label="'续费规则'" prop="renewType">
					...
					<span v-if="addData.renewType === 0" style="display: inline-block;line-height: 20px;color: #999;">
						<i class="el-icon-warning"></i>常规续费：用户选择续费时长(月数X)，若当前已过期则从当前时间开始续费X个月，若当前未过期则按实际过期时间开始续费X个月。
					</span>
					<span v-if="addData.renewType === 1" style="display: inline-block;line-height: 20px;color: #999;">
				...
				<el-form-item label-width="120px" label="是否开通虚拟卡" prop="virtualCardEnabled">
					<el-switch v-model="addData.virtualCardEnabled"
						active-text="开启"
						inactive-text="关闭"
						active-color="#67c23a">
					</el-switch>
					<span v-if="!addData.virtualCardEnabled" style="display: inline-block;margin-left: 10px;color: #999;">
