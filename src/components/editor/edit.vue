<template>
	<div class="editorBox" v-loading="quillUpdateImg">
		<el-upload style="display:none" class="avatar-uploader" :action="serverUrl" name="file" :show-file-list="false"
			:on-error="uploadError" :before-upload="beforeUpload" :http-request="uploadSuccess"></el-upload>
		<el-upload style="display:none" class="video-uploader" :action="serverUrl" name="file" :show-file-list="false"
	...
	export default {
						...
						toolbar: {
							...
							handlers: {
								...
								'video': function(value) {
									if (value) {
										// 触发input框选择图片文件
										document.querySelector(".editorBox .video-uploader input").click();
									} else {
										this.quill.format("image", false);
									}
								}
		...
		methods: {
			...
			VuploadSuccess(file) { // res为图片服务器返回的数据
				this.quillUpdateImg = true;
				console.log(this.quillUpdateImg)
				// 获取富文本组件实例
				let quill = this.$refs.myQuillEditor.quill;
				var formData = new FormData(); //构造一个 FormData，把后台需要发送的参数添加
				formData.append("file", file.file); //接口需要传的参数
